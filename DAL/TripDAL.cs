...
namespace DAL
{
	public class TripDAL : ITripDAL
	{
		...
		public void Insert(Trip obj)
		{
			using (SqlConnection conn = new SqlConnection(GetConnectionString()))
			{
				var strSql = @"BusinessTravel.CreateTrip";
				var param = new
				{
		...
		public decimal CardTotalExpense()
		{
			...
			using (SqlConnection conn = new SqlConnection(GetConnectionString()))
			{
				var strSql = "BusinessTravel.DashboardTotalExpense";
				try
				{
					conn.Open();
					SqlCommand cmd = new SqlCommand(strSql, conn);
					cmd.CommandType = CommandType.StoredProcedure;
					totalExpense = Convert.ToDecimal(cmd.ExecuteScalar()); // Mengambil hasil menggunakan ExecuteScalar
				}
				catch (Exception ex)
				{
					throw new Exception("Error CardTotalExpense DAL: " + ex.Message);
				}
			}
		...
		public int CardTotalApprovalRequest()
		{
			...
			using (SqlConnection conn = new SqlConnection(GetConnectionString()))
			{
				var strSql = "BusinessTravel.DashboardTotalRequest";
				try
				{
					conn.Open();
					SqlCommand cmd = new SqlCommand(strSql, conn);
					cmd.CommandType = CommandType.StoredProcedure;
					totalRequest = (int)cmd.ExecuteScalar(); // Mengambil hasil menggunakan ExecuteScalar
				}
				catch (Exception ex)
				{
					throw new Exception("Error retrieving total request from database: " + ex.Message);
				}
			}
		...
		public IEnumerable<Trip> GetAllOnlyInProgress()
		{
			using (SqlConnection conn = new SqlConnection(GetConnectionString()))
			{
				var strSql = "SELECT t.*, s.StatusName, st.Name FROM BusinessTravel.Trip AS t JOIN BusinessTravel.Status As s ON t.StatusID = s.StatusID JOIN BusinessTravel.Staff As st ON t.SubmittedBy = st.StaffID WHERE t.IsDeleted = 0 AND t.StatusID = 2";

				try
				{
					List<Trip> trips = new List<Trip>(); // Inisialisasi List<Trip>
					SqlCommand cmd = new SqlCommand(strSql, conn);
					conn.Open();
					SqlDataReader dr = cmd.ExecuteReader();
					if (dr.HasRows)
